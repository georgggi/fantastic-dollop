import pandas as pd; df = pd.read_csv('ObesityDataSet_raw_and_data_sinthetic.csv')
...
clV = v(
    estimators=[('DT', clDT), ('KNN', clKNN), ('RF', clRF)],
    voting='hard'
)
clV.fit(x_train, y_train)
VC_acc, y_pred = clV.score(x_test, y_test), clV.predict(x_test)
print(f'Accuracy VC: {VC_acc * 100:.1f}%. \nΔ(VC, DT) = {(VC_acc - DT_acc) * 100:.1f}%; ',
      f'Δ(VC, KNN) = {(VC_acc - KNN_acc) * 100:.1f}%; Δ(VC, RF) = {(VC_acc - RF_acc) * 100:.1f}%')
